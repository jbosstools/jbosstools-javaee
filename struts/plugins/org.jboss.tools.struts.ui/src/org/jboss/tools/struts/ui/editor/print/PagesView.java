...
//import org.jboss.tools.struts.ui.diagramview.CommentView;
//import org.jboss.tools.struts.ui.diagramview.CommentsCorner;
//import org.jboss.tools.struts.ui.diagramview.DrawElement;
//import org.jboss.tools.struts.ui.diagramview.HeadElement;
//import org.jboss.tools.struts.ui.diagramview.LineElement;
//import org.jboss.tools.struts.ui.diagramview.ProcessItemView;
//import org.jboss.tools.struts.ui.diagramview.TailElement;
...
public class PagesView extends Composite implements MouseListener,PaintListener{
		 ...
		 PropertyChangeListener pcl = new PropertyChangeListener(){
			 public void propertyChange(PropertyChangeEvent e){
				  if(e.getPropertyName().equals("scale")){
						setViewScale(calculateScale(containerSize,pages.getDimension()));
						//img = null;
  ...
  Color yellowColor = new Color(null, 0xff, 0xf6, 0xcb);
	...
	for(int i=0; i<pages.getSourcePagesCount(); i++){
		   Rectangle nrz = new Rectangle((int)Math.round(pages.getSourcePage(i).getRectangle().x*viewScale),(int)Math.round(pages.getSourcePage(i).getRectangle().y*viewScale),(int)Math.round(pages.getSourcePage(i).getRectangle().width*viewScale),(int)Math.round(pages.getSourcePage(i).getRectangle().height*viewScale));
		   if((nrz.y+nrz.height)>ymax){
			ymax = nrz.y+nrz.height;
			//yy.add(new Integer(ymax));
			if(recount)pH = nrz.height;
			cy++;
		   }
		   if((nrz.x+nrz.width)>xmax){
			xmax = nrz.x+nrz.width;
			//xx.add(new Integer(xmax));
			if(recount)pW = nrz.width;
			cx++;
		   }
	}
	...
	if(figures.isEmpty())getChilds(f);
	...
	for(int i=0; i<figures.size();i++){
			...
			if(f1.getBounds().x<0){
				if(f1.getBounds().x<ix)ix=f1.getBounds().x; 
			}
			if(f1.getBounds().y<0){
				if(f1.getBounds().y<iy)iy=f1.getBounds().y; 
			}
	}
	
//	int izeroX = zeroX;
//	int izeroY = zeroY;
//	zeroX=Math.abs(ix)+zeroX;
//	zeroY=Math.abs(iy)+zeroY;
	
	for(int i=0; i<figures.size();i++){
		IFigure f1 = (IFigure)figures.get(i);
		Rectangle r = new Rectangle(f1.getBounds().x+Math.abs(ix),f1.getBounds().y+Math.abs(iy),f1.getBounds().width,f1.getBounds().height);
	//	Rectangle r = new Rectangle(f1.getBounds().x,f1.getBounds().y,f1.getBounds().width,f1.getBounds().height);
		Rectangle rs = new Rectangle((int)(r.x*viewScale)+zeroX,(int)(r.y*viewScale)+zeroY,(int)(r.width*viewScale),(int)(r.height*viewScale));
		...
		if(f1 instanceof PolylineConnection){
			...
			for(int j=0; j<pl.size()-1; j++){
				Point p = new Point(pl.getPoint(j).x+Math.abs(ix),pl.getPoint(j).y+Math.abs(iy));
				Point p1 = new Point(pl.getPoint(j+1).x+Math.abs(ix),pl.getPoint(j+1).y+Math.abs(iy));
				g2.setBackground(white);
				g2.setForeground(gray);
				g2.setLineStyle(SWT.LINE_SOLID);
				g2.drawLine((int)(p.x*viewScale)+zeroX,(int)(p.y*viewScale)+zeroY,(int)(p1.x*viewScale)+zeroX,(int)(p1.y*viewScale)+zeroY);	
			}
		}else{
			if(f1 instanceof ProcessItemFigure){
				ProcessItemFigure group = (ProcessItemFigure)f1;
				g2.setForeground(black);
					if(group.processItem.isConfirmed()){
					//	g2.setForeground(yellowColor);
						g2.setBackground(yellowColor);
					}else{ 
					//	g2.setForeground(lightGrayColor);
...
//					int dm = FigureUtilities.getTextWidth(s, ff);
					FontData fd = ff.getFontData()[0];
//					int h = fd.getHeight();
					...
					if(nn>0){
						...
						//int dmm = FigureUtilities.getTextWidth(s, f2);
						//Font f2 = new Font(pe.display,"default",nh,1);
						g2.setFont(f2);
						
					//	g2.setBackground(blue);
					//	g2.fillRectangle(new Rectangle(rs.x,rs.y,rs.width,rs.height));
	...
	img = new Image(cmp.getDisplay(),1024,1024);
	GC gg = new GC(img);
	SWTGraphics sg = new SWTGraphics(gg);
	...
	img2 = new Image(cmp.getDisplay(),img.getImageData());
   }
	g2.drawImage(img2,0,0,img.getImageData().width,img.getImageData().height,0,0,(int)(img.getImageData().width*viewScale),(int)(img.getImageData().height*viewScale));
	...
	*/
	...
	for(int i=0; i<yy.size();i++){
		for(int j=0; j<xx.size();j++){
			rec.add(new Rectangle(((Integer)xx.get(j)).intValue()+zeroX,((Integer)yy.get(i)).intValue()+zeroY,pW,pH));	
		}
	}
	Object[] rn = rec.toArray();
		 ...
		 for(int i=0; i<pages.getSourcePagesCount(); i++){
			...
			//Rectangle nr = new Rectangle((int)(pages.getSourcePage(i).getRectangle().x*viewScale),(int)(pages.getSourcePage(i).getRectangle().y*viewScale),(int)(pages.getSourcePage(i).getRectangle().width*viewScale),(int)(pages.getSourcePage(i).getRectangle().height*viewScale));
			//Rectangle nr = new Rectangle((int)(pages.getSourcePage(i).getRectangle().x*viewScale),(int)(pages.getSourcePage(i).getRectangle().y*viewScale),(int)(pW*viewScale),(int)(pH*viewScale));
			if(!pages.getSourcePage(i).isSelected()){
				Rectangle sr = pages.getSourcePage(i).getRectangle();
				Point p = new Point((int)Math.round(sr.x*viewScale+pW/2+zeroX),(int)Math.round(sr.y*viewScale+pH/2+zeroY));
				for (int j=0; j<rn.length; j++){
					Rectangle rnt = (Rectangle)rn[j]; 
					if(rnt.contains(p)){
						g2.setXORMode(true);
						g2.setBackground(pe.display.getSystemColor(SWT.COLOR_DARK_GRAY));
						Rectangle nr = new Rectangle(rnt.x,rnt.y,pW+1,pH+1);
						g2.fillRectangle(nr);
						g2.setXORMode(false);						   
					}
				}
   ...
   public Dimension getPreferredSize(){
	  return new Dimension((int)(pages.getDimension().width*viewScale),(int)(pages.getDimension().height*viewScale));
   }
   ...
   public void mouseDown(MouseEvent e){
	if(selectionEnabled){
			 ...
			 for(int i=0; i<this.pages.getSourcePagesCount(); i++){
				Rectangle nr = new Rectangle((int)(pages.getSourcePage(i).getRectangle().x*viewScale)+zeroX,(int)(pages.getSourcePage(i).getRectangle().y*viewScale)+zeroY,(int)(pages.getSourcePage(i).getRectangle().width*viewScale),(int)(pages.getSourcePage(i).getRectangle().height*viewScale));
				if(nr.contains(p)){
				   pages.getSourcePage(i).setSelected(!pages.getSourcePage(i).isSelected());
				  this.redraw();
				}
			 }
		  }
