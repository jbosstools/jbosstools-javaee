...
public class ConnectionFigure extends PolylineConnection implements Connection, AnchorListener{
	/*public void setConnectionRouter(ConnectionRouter cr) {
		if(cr != null && !(cr instanceof StrutsConnectionRouter))Thread.dumpStack();
	...
	private int oldStartPointX=0, oldStartPointY=0, oldEndPointX=0, oldEndPointY=0;
	...
	/*public void refreshFont(){
		if(getChildren().size() > 0 && getChildren().get(0) instanceof Label){
			...
			label.setFont(editPart.getLinkModel().getStrutsModel().getOptions().getPathFont());
	...
	public void save(){
		if(editPart != null)editPart.save();
	}
