...
public StrutsConnectionDragTracker(ConnectionEditPart cep, boolean vertical, int id) {
	...
	list = getConnection().getPoints();//.getCopy();
	index1 = id;
	index2 = id+1;
...
protected boolean handleDragInProgress(){
	...
	if(vertical){
		...
		p2.x -= p2.x%8;
		if(index1 == 1 && p1.x < list.getPoint(0).x){
			p1.x = list.getPoint(0).x;
			p2.x = list.getPoint(0).x;
		}
		if(index2 == list.size()-2 && p1.x > list.getPoint(list.size()-1).x){
			p1.x = list.getPoint(list.size()-1).x;
			p2.x = list.getPoint(list.size()-1).x;
		}
...
protected boolean handleButtonUp(int button){
	if (stateTransition(STATE_DRAG_IN_PROGRESS, STATE_TERMINAL)){
		...
		//((ILink)getConnectionEditPart().getModel()).save();
	}
	return true;
}
