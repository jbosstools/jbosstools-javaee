...
public class JSFProjectBeans extends RegularObjectImpl implements WebProjectNode {
	...
	private void process(XModelObject[] bs, BeanConstants constants,
		...
		for (int j = 0; j < bs.length; j++) {
			...
			if(b == null) {
				b = (JSFProjectBean)getModel().createModelObject(JSFProjectBean.ENTITY, null);
				b.setAttributeValue(XModelObjectConstants.ATTR_NAME, bn);
				b.setBeans(this);
				addChild(b);
			} else {
				map.remove(bn);
			...
			b.setAttributeValue(JSFProjectBean.ATTR_CLASS_NAME, cn);
			b.addBean(bs[j]);
			b.setType(type);
			classes.put(cn, b);
		}
	...
	private void updateListener() {
		...
		if(EclipseResourceUtil.getProject(this) == null) return;
