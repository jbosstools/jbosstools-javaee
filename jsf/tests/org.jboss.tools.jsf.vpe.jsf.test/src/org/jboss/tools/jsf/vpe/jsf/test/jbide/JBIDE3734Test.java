...
public class JBIDE3734Test extends VpeTest {
	...
	public void testLink() throws Throwable {
		setException(null);
		VpeController vpeController = openTestPage();
		nsIDOMElement head = queryInterface(vpeController.getVisualBuilder().getHeadNode(), nsIDOMElement.class);

		nsIDOMNodeList links = head.getElementsByTagName(HTML.TAG_STYLE);
		boolean pageCssLinkFound = false;
		for (int i = 0; i < links.getLength(); i++) {
			final String href = queryInterface(links.item(i), nsIDOMElement.class).getAttribute(HTML.ATTR_HREF);
	...
	private VpeController openTestPage() throws CoreException, PartInitException, IOException {
