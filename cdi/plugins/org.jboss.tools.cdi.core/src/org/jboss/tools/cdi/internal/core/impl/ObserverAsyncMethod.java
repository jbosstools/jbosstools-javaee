...
public class ObserverAsyncMethod extends BeanMethod implements IObserverMethod {
	...
	protected Parameter newParameter(ParameterDefinition p) {
		if(p.isAnnotationPresent(CDIConstants.OBSERVERS_ASYNC_ANNOTATION_TYPE_NAME)) {
	...
	public Collection<IParameter> getObservedParameters() {
		...
		for (IParameter p: parameters) {
			if(p.isAnnotationPresent(CDIConstants.OBSERVERS_ASYNC_ANNOTATION_TYPE_NAME)) result.add(p);
		}
		return result;
	}
